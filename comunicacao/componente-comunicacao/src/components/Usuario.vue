<template>
    <div class="container">
        <h1>Componente Usuário</h1>
        <p>Esse é um componente muito legal!</p>
        <p>Nome é <strong>{{nome}}</strong></p>
        <p>Idade é <strong>{{idade}}</strong></p>
        <button @click="alterarNome">Alterar Nome</button>
        <hr>
        <div class="componentes">
            <!--PASSANDO PROPRIEDADES DO PAI PARA O FILHO-->
            <!--EMITINDO EVENTO DO FILHO PARA O PAI-->
            <app-usuario-info 
            :nome="nome"
            @nomeMudou = "nome = $event"
            :reiniciarFn="reiniciarNome"
            :idade="idade"
            />
            <app-usuario-editar :idade ="idade"
            />
        </div>
    </div>
</template>

<script>
//IMPORTANDO COMPONENTES FILHO
import AppUsuarioInfo from './UsuarioInfo'
import AppUsuarioEditar from './UsuarioEditar'

export default {
    //INSTANCIANDO COMPONENTES FILHO
    components: { AppUsuarioInfo, AppUsuarioEditar },
    //DADOS DO PAI A SER PASSADO PARA OS FILHOS CASO NECESSÁRIO
    data(){
        return {
            nome:'Pedro',
            idade: 21
            }
    },
    //METHODOS A SER RESOLVIDOS
    methods:{
        alterarNome(){
            this.nome = 'Ana'
        },
        reiniciarNome(){
            this.nome = 'Pedro'
        }
    }
}
</script>

<style scoped>
    .container {
        background-color: #27363b;
        color: #fff;
        padding: 10px;
    }

    .container hr {
        margin: 20px 10px;
    }

    .componentes {
        display: flex;
    }

    .componentes > * {
        margin: 10px;
    }
</style>
